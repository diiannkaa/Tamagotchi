...
public class NeedsController : MonoBehaviour
{
    ...
    public int TickAmountSinceLastTimeToCurrentTime(DateTime lastTime, float tickRateInSeconds)
    {
        ...
        if (dayDifferenceSecondsAmount > 0) return Mathf.RoundToInt(dayDifferenceSecondsAmount / tickRateInSeconds);
    ...
    {
        if (food < 50 && happiness < 50 && energy < 50)
        {
            petController.Tired();
        }
        else
        {
            petController.NotTired(); 
        }
    }
}
